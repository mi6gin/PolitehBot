...
async def get_plan_info(cookies, user_id):
    ...
    async with aiohttp.ClientSession(cookies=cookies) as session:
        ...
        async with session.get("https://univer.kstu.kz/teacher/myschedule/",
                               headers=headers.Language(language)) as response:
            if response.status == 200:
                soup = BeautifulSoup(await response.text(), "html.parser")

                tables = soup.find_all('table', class_='mt')

                # Для каждой таблицы найдем все ссылки, удовлетворяющие условию
                for table in tables:
                    links = table.find_all('a',
                                           href=lambda href: href and href.startswith('/teacher/myschedule/print/'))
                    # Выведем найденные ссылки
                    for link in links:
                        url = link['href']
                        await get_plan(cookies,
                                       user_id, url)
...
async def get_plan(cookies, user_id, url):
    headers = HTTPHeaders(user_id, user_id, user_id, user_id, user_id)
    async with aiohttp.ClientSession(cookies=cookies) as session:
        language = await set_lang(user_id)
        async with session.get(f"https://univer.kstu.kz/{url}",
                               headers=headers.Language(language)) as response:
            if response.status == 200:
                soup = BeautifulSoup(await response.text(), "html.parser")

...
async def get_pdfka_info(cookies, user_id, id):
    ...
    async with aiohttp.ClientSession(cookies=cookies) as session:
        ...
        async with session.get(f"https://univer.kstu.kz{id}", headers=headers.Language(language)) as response:
            if response.status == 200:
                ...
                file = BufferedInputFile(pdf_document, filename='Unknow.pdf')
                ...
                if cookies is not False:
                    user = await get_pdfka_info(cookies, user_id, id)
                    return user

async def Pdfka(user_id, id):
    cookies = await AuthVer1.Authorization(user_id)
    if cookies is not False:
        std_data = await get_pdfka_info(cookies, user_id, id)
        return std_data
    else:
        cookies = await AuthVer2.Authorization(user_id)
        if cookies is not False:

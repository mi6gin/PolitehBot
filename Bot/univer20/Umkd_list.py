...
async def get_umkd_download(cookies, message: types.Message, user_id, id):
    ...
    async with aiohttp.ClientSession(cookies=cookies) as session:
        async with session.get(f'https://univer.kstu.kz/student/umkd/{user_id}') as response:
                ...
                if teacher_rows:
                    ...
                    for teacher_row in teacher_rows:
                        teacher_name_element = teacher_row.find('td', class_='ct',
                                                                text=lambda text: 'Преподаватель:' in text or'Faculty member:' in text or 'Оқытушы:' in text)
